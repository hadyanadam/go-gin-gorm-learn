...
func (c *bookController) Insert(context *gin.Context) {
	...
	if errDTO != nil {
		...
		authHeader, _ := helper.SplitHeader(context.GetHeader("Authorization"), "Bearer")
...
func (c *bookController) Update(context *gin.Context) {
	...
	if c.bookService.IsAllowedToEdit(userID, bookUpdateDTO.ID) {
		...
		response := helper.BuildErrorResponse("You don't have permission", "You not owner", helper.EmptyObj{})
